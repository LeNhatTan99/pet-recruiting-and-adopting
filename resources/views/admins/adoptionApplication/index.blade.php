              ...
              <table class="table table-striped ">
                ...
                <tbody>
                  ...
                  <tr class="item-row">
                    ...
                    <td class="column-text break-word align-middle">{{ $value->reason }}</td>
                    ...
                    <td class="align-middle">
                      <form id="myForm">
                        <select id="selectStatus" name="status" data-value-id="{{ $value->id }}" class="form-control search-slt font-weight-300">
                            @foreach ($status as $valueStatus)
                                <option value="{{ $valueStatus }}"
                                    @if (old('status', $value->status ) == $valueStatus) selected @endif>{{ $valueStatus }}</option>
                            @endforeach
                        </select>
                      </form>
                  </td>
